(function() {
    ...
    LastPass.prototype.generate_and_save_password = function (domain, username, length, include_symbols=true) {
        utils.debug("using lastpass to generate password, and then save that password against the supplied username and domain");
    ...
    LastPass.prototype.get_username_and_password = function(siteId) {
        utils.assertNotEmpty(siteId, 'siteId');
        utils.debug(`retrieving username and password for site with id ${siteId}`);
    ...
    LastPass.prototype.get_command = function(comm, input){
        ...
        //not logged in
            ...
            utils.debug(`trying to log into lastpass`);
    ...
    LastPass.prototype.search_lpass_for_domain = function(domain) {
        utils.debug(`searching lastpass for the provided domain: ${domain}`);
        utils.assertNotEmpty(domain, 'domain');

        var results = yield this.get_command(`lpass show --id -G ${domain}`);
        var matches = results.data.split("\n");
        if (matches.length > 1){
            utils.debug(`${matches.length} matches found for ${domain}, filtering results to only ones with ids in the name`);
    ...
    LastPass.prototype.get_site_id_for_domain = function(domain){
        utils.debug(`retrieving site id from lastpass by searching for the provided domain: ${domain}`);
        utils.assertNotEmpty(domain, 'domain');

        var matches = yield this.search_lpass_for_domain(domain);
        var id;
        if (matches.length == 1){
            utils.debug(`only one match found`);
            id = matches[0];
        }
        else {
        ...
        utils.debug(`1 match found or selected: $(id)`);
        yield co_return(id);
    };

    LastPass.prototype.set_login_and_password_fields = function(siteId){
        utils.assertNotEmpty(siteId, 'siteId');
        utils.debug(`trying to set login and password fields in HTML for site ${siteId}`);
    ...
    LastPass.prototype._get_lastpass_value = function(siteId, type, masterpassword){
        utils.assertNotEmpty(siteId, 'siteId');
        utils.debug(`querying lastpass site ${siteId} for ${type}`);
    ...
    LastPass.prototype._get_fields = function(siteId) {
        utils.assertNotEmpty(siteId, 'siteId');
        utils.debug(`retrieving HTML field names and values for site ${siteId}`);
